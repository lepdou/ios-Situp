...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    if (max < defaultYPieceCount) {
        max = defaultYPieceCount;
    }
    if (self.showRange) {
        _yValueMin = min;
    }else{
    ...
    float level = (_yValueMax-_yValueMin) /((_yPieceCount -1)*1.0);
    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;
    CGFloat levelHeight = chartCavanHeight /((_yPieceCount -1)*1.0);
    
    for (int i=0; i<_yPieceCount; i++) {
...
-(void)strokeChart
{
    for (int i=0; i<_yValues.count; i++) {
        ...
        NSInteger max_i = NSIntegerMin;
        NSInteger min_i = NSIntegerMax;
        ...
        for (NSString * valueString in childAry) {
            ...
            BOOL isShowMaxAndMinPoint;
            if (!self.showPointValue) {//是否显示最大值和最小值
                if (self.ShowMaxMinArray) {
                    isShowMaxAndMinPoint = YES;
                    if ([self.ShowMaxMinArray[i] intValue]>0) {
