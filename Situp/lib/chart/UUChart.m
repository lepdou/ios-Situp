...
-(void)setUpChart{
    if (self.chartStyle == UUChartLineStyle) {
        ...
        if ([self.dataSource respondsToSelector:@selector(UUChart_yPieceCount:)]) {
            [_lineChart setYPieceCount:[self.dataSource UUChart_yPieceCount:self]];
        }
        //选择标记范围
        ...
        if ([self.dataSource respondsToSelector:@selector(UUChart:ShowHorizonLineAtIndex:)]) {
            ...
            for (int i=0; i<_lineChart.yPieceCount; i++) {
        ...
        //显示点的值
        if ([self.dataSource respondsToSelector:@selector(showPointValue:)]) {
            _lineChart.showPointValue = [self.dataSource showPointValue:self];
        }
        //判断显示最大最小值
